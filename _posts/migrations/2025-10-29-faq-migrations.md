...
To set or change the metadata of your instance, go to the Horizon interface Project > Compute > Instances. Click the more options triangle next to the instance and choose `Update Metadata`. Open `Provider platform options` > `Migration` 
...
and Click `+` on `Scheduling options`. Update the date/timestamp 
